...
public class Business {
    ...
    /**
     * Typically Cards are good for 3 years. Will create a date between 24 and 48 months
     * @return
     */
    public static Date creditCardExpiryDate(){
        DateTime dateTime = new DateTime();
        DateTime addedMonths = dateTime.plusMonths(Utils.randomIntBetweenTwoNumbers(24,48));
    ...
    public static String creditCardNumber(CreditCardType type) {
        ...
        switch (type) {
            case VISA: {
                ...
                tempCC.append(Utils.randomNumberString(14));
                ccNumber = tempCC.append(+generateCheckDigit(tempCC.toString())).toString();
                break;
            }
            case MASTERCARD: {
                StringBuffer tempCC =  new StringBuffer("5");
                tempCC.append(Utils.randomIntBetweenTwoNumbers(1,5));
                tempCC.append(Utils.randomNumberString(13));
                ccNumber = tempCC.append(+generateCheckDigit(tempCC.toString())).toString();
                break;
            }
            case AMEX: {
                StringBuffer tempCC =  new StringBuffer("3");
                tempCC.append(Utils.randomIntBetweenTwoNumbers(4,7));
                tempCC.append(Utils.randomNumberString(12));
                ccNumber = tempCC.append(+generateCheckDigit(tempCC.toString())).toString();
                break;
            }
            case DISCOVER: {
                StringBuffer tempCC =  new StringBuffer("6011");
                tempCC.append(Utils.randomNumberString(11));
                ccNumber = tempCC.append(+generateCheckDigit(tempCC.toString())).toString();
                break;
            }
        }
